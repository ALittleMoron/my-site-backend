...
def test_class_as_dict() -> None:
    ...
    with pytest.raises(TestHttpException1) as exc_info:
        raise TestHttpException1().from_template(test='test').with_attr(attr).with_loc(loc)
...
def test_class_as_dict_with_params() -> None:
    ...
    assert exc_info.value.as_dict(attr, loc, test='test') == test_http_exception1_dict
